...
if (!isset($_GET['id'])) { header("Location: dashboard_landlord.php"); exit(); }
...
if (isset($_POST['update_room'])) {
    ...
    // FETCHING THE SHARED PRICE FROM THE FORM
    $shared_price = !empty($_POST['price_shared']) ? $_POST['price_shared'] : 0;
    
    // Added safety checks for wifi_type and water_type in case they are missing from the form
    $wifi_type = $_POST['wifi_type'] ?? '';
    $water_type = $_POST['water_type'] ?? '';

    // UPDATED QUERY TO USE shared_price
    $stmt = $conn->prepare("UPDATE properties SET title=?, location=?, description=?, contact_phone=?, contact_facebook=?, contact_email=?, wifi_type=?, water_type=?, price=?, shared_price=?, inclusions=?, paid_addons=?, images=?, status=? WHERE id=?");
    $stmt->bind_param("ssssssssddssssi", $_POST['title'], $_POST['location'], $_POST['description'], $_POST['contact_phone'], $_POST['contact_facebook'], $_POST['contact_email'], $wifi_type, $water_type, $price, $shared_price, $inclusions, $paid_addons, $images_json, $_POST['status'], $prop_id);
    $stmt->execute();
    ...
    header("Location: dashboard_landlord.php?msg=Updated"); exit();
}
...
<head>
    ...
    <style>
        ...
        .room-thumb-wrapper { 
            width: 200px;   /* Significantly wider */
            height: 140px;  /* Taller to match form height */
            background: #000; 
            border-radius: 6px; 
            overflow: hidden; 
            position: relative; 
...
<form method="POST" enctype="multipart/form-data">
    ...
    <div class="page-header">
        ...
        <a href="dashboard_landlord.php" class="back-link"><i class="bi bi-arrow-left"></i> Back to Dashboard</a>
    </div>
        ...
        <div class="col-lg-7">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="status" class="form-select">
                            <option value="Active" <?php if($prop['status']=='Active') echo 'selected'; ?>>Active</option>
                            <option value="Occupied" <?php if($prop['status']=='Occupied') echo 'selected'; ?>>Full</option>
                            <option value="Hidden" <?php if($prop['status']=='Hidden') echo 'selected'; ?>>Hidden</option>
                        </select>
                    </div>
                </div>
                <div class="mb-3">
                 ...
                 <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="price_shared" class="form-control" value="<?php echo $prop['shared_price']; ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="edit-card">
                ...
                <div>
                    ...
                    <div>
                    <?php 
                    $paid = ["Drinking Water", "Refrigerator Use", "Rice Cooker", "Heater", "Laptop", "Fan"];
                    foreach($paid as $opt) {
    ...
    <div class="sticky-footer">
        <a href="dashboard_landlord.php" class="btn btn-outline-secondary border-0 text-white d-flex align-items-center">Cancel</a>
