...
// 3. RECENT ACTIVITY
$recent_users = $conn->query("SELECT * FROM users ORDER BY created_at DESC LIMIT 6"); // Increased to 6 to fit grid nicely
?>
...
<head>
    ...
    <style>
        ...
        /* USER BOX UI */
        .user-box-container { padding: 20px 0; }
        .user-box { background: #222; border: 1px solid #333; border-radius: 16px; padding: 20px; transition: 0.2s; height: 100%; display: flex; flex-direction: column; justify-content: space-between;}
        .user-box:hover { border-color: #555; transform: translateY(-3px); }
        .user-img { width: 45px; height: 45px; border-radius: 50%; object-fit: cover; margin-right: 15px; border: 2px solid #333; }
        .badge-role { font-size: 11px; padding: 5px 12px; border-radius: 20px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
...
<div class="sidebar">
    ...
    <a href="dashboard_admin.php" class="nav-link active"><i class="bi bi-speedometer2"></i> Dashboard</a>
...
<div class="main-content">
    ...
    <div class="row g-4 mb-5">
        <div class="col-md-3">
            <div class="stat-card">
                <div class="stat-icon"><i class="bi bi-person-tie"></i></div>
    ...
    <div class="user-box-container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="admin_users.php?role=All" class="btn btn-sm btn-outline-secondary border-0 text-white">View All <i class="bi bi-arrow-right"></i></a>
        </div>
        
        <div class="row g-4">
            <?php while($u = $recent_users->fetch_assoc()): 
                $u_pic = !empty($u['profile_pic']) ? "assets/uploads/".$u['profile_pic'] : "assets/default.jpg";
                $badge = ($u['role'] == 'Landlord') ? 'background: rgba(13, 202, 240, 0.15); color: #0dcaf0; border: 1px solid rgba(13, 202, 240, 0.3);' : 'background: rgba(255, 193, 7, 0.15); color: #ffc107; border: 1px solid rgba(255, 193, 7, 0.3);';
            ?>
            <div class="col-xl-4 col-md-6">
                <div class="user-box">
                    <div class="d-flex align-items-center mb-4">
                        ...
                        <div>
                            <div class="fw-bold text-white fs-6"><?php echo htmlspecialchars($u['first_name'] . ' ' . $u['last_name']); ?></div>
                            <div class="small text-secondary" style="font-size: 12px;"><?php echo htmlspecialchars($u['email']); ?></div>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between align-items-center pt-3 border-top border-secondary border-opacity-25">
                        <span class="badge-role" style="<?php echo $badge; ?>"><?php echo $u['role']; ?></span>
                        <span class="small text-secondary" style="font-size: 11px;">Joined: <?php echo date("M d, Y", strtotime($u['created_at'])); ?></span>
                    </div>
                </div>
            </div>
            <?php endwhile; ?>
